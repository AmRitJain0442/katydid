...
def pull_payload(action: str = "synchronize", number: int = 7, base: str = "main") -> bytes:
    return encoded(
        repository_payload(
            action=action,
            number=number,
...
def test_pull_request_reconciles_current_state_deduplicates_and_closes(tmp_path: Path) -> None:
    ...
    second = ingress.ingest("pull_request", "delivery-pr-2", pull_payload("closed"))
    ...
    third_body = pull_payload("synchronize")
